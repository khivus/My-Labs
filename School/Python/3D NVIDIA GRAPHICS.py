...
ent_kringe=Entry(root,width=5)
ent_kringe.insert(0,0.25)
ent_kringe.place(x=520,y=50)
...
def but_clk():
    global minx,miny,maxx,maxy,mx,my,a,b,c,alpha,kringe
    ...
    kringe=float(ent_kringe.get())
    mx=cx/(maxx-minx)
    my=cy/(maxy-miny)
    
    #рисуем обычными координатами оси
    while rbt.get()!=0:
        
        canv.delete(ALL)
        canv.create_line(p(0,maxy),p(0,miny),arrow=FIRST,fill='gray50')
        line3d(0,0,miny,0,0,maxy,None,'gray50',LAST)
        line3d(minx,0,0,maxx,0,0,None,'gray50',LAST)
        #3д рисунок
        
        if rbt.get()==1: #куб
            line3d(0,0,0,0,0,10)    #ось ox под углом
            line3d(0,0,0,0,10,0)    #ось oz вверх
            line3d(0,0,0,10,0,0)    #ось oy вправо
...
but_stop=Button(root,text='стоп',command=stop)
but_stop.place(x=520,y=20)
