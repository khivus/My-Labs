...
result_struct solution(double accuracy) { // solution function
    ...
    if (abs(x_next - x_prev) < accuracy) {
        ...
        x_prev = x_next;
        n++;
        return solution(accuracy); // recursion
    }
}
...
int main() {
    int point, write_in; // declaring variables
    ...
    } while (point != 7); // when '7' pressed program will be completed
    return 0;
}